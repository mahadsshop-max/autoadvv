...
// check a token is valid and grab the account info
...
class DiscordSelfBot {
    ...
    handleDM(data) {
        // DMs have no guild_id. Channel type 1 = DM, 3 = group DM.
        if (data.guild_id) return;
        if (data.channel_type === 3) return; // skip group DMs
        ...
        // Optimistically mark as replied BEFORE the async send to prevent
        // a flood of duplicate replies if the user sends multiple messages
        // before our reply lands.
        ...
        setTimeout(() => {
            this.sendDM(data.channel_id, this.autoReplyText).then(() => {
                ...
                console.log(`[SELFBOT ${this.configId}] Auto-replied to ${data.author.username} (${data.author.id})`);
            }).catch(err => {
                // Reply failed - roll back so we try again next time
                this.repliedUsers.delete(data.author.id);
                console.error(`[SELFBOT ${this.configId}] Auto-reply failed:`, err.response?.status, err.message);
            });
    ...
    identify() {
        // IMPORTANT: user (selfbot) accounts must NOT send the `intents` field.
        // Sending intents=512 (GUILD_MESSAGES only) is what was silently dropping
        // every DM_CREATE event, which is why auto-DM "doesn't DM".
        // Omitting intents tells Discord to deliver everything to the user account.
        const payload = {
            op: 2,
            d: {
                token: this.token,
                capabilities: 16381, // standard web client capabilities
                properties: {
                    os: 'Windows',
                    browser: 'Chrome',
                    device: '',
